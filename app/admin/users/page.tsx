...
export default function AdminUsersPage() {
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-slate-400">アクセス権限がありません</div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-slate-950">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            {isAdmin && (
              <button
                ...
                className="bg-indigo-600 text-slate-50 py-2 px-4 rounded-md hover:bg-indigo-700 transition-colors"
              >
                社員を追加
              </button>
            )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 rounded-lg shadow-md max-w-md w-full">
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-slate-800"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="h-4 w-4 text-indigo-400 border-slate-700 rounded focus:ring-indigo-500"
                  />
                  <label htmlFor="canViewAllApplications" className="text-sm font-medium text-slate-300">
                    全社員の申請を閲覧可能にする
                  </label>
                </div>
                <div className="flex gap-4 pt-4">
                  <button
                    type="button"
                    onClick={() => setShowModal(false)}
                    className="flex-1 bg-slate-700 text-slate-300 py-2 px-4 rounded-md hover:bg-slate-600 transition-colors"
                  >
                    キャンセル
                  </button>
                  <button
                    type="submit"
                    disabled={processing}
                    className="flex-1 bg-indigo-600 text-slate-50 py-2 px-4 rounded-md hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {processing ? '保存中...' : '保存'}
                  </button>
                </div>
